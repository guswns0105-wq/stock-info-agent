...
def collect_youtube_channel(src):
    ...
    try:
        ...
        for e in root.findall('a:entry',ns)[:5]:
            ...
            out.append({'source':src['name'],'source_type':'youtube_channel','region':src.get('region','domestic'),'title':title,'summary':desc[:500],'url':'https://youtu.be/'+vid if vid else src['url'],'published_at':published,'text':title+' '+desc})
    except Exception as e:
        out.append({'source':src['name'],'source_type':'youtube_channel','region':src.get('region','domestic'),'title':'수집 실패','summary':str(e),'url':src['url'],'published_at':'','text':''})
    return out

def collect_youtube_video(src):
    vid=src.get('video_id') or src['url'].split('youtu.be/')[1].split('?')[0]
    title=yt_oembed(vid) or src['name']
    return [{'source':src['name'],'source_type':'youtube_video','region':src.get('region','global'),'title':title,'summary':'YouTube transcript API가 차단될 수 있어 현재 배치에서는 제목/메타데이터 기반으로 1차 분석합니다. 채널 RSS 또는 자막 접근이 가능해지면 본문 기반으로 강화합니다.','url':'https://youtu.be/'+vid,'published_at':'','text':title}]
...
def main():
    ...
    for r in records:
        ...
        items.append({'region':region,'source':r.get('source'), 'title':title, 'summary':summary[:500], 'tickers':[m['ticker'] for m in mentions], 'recommendation':'공통관심 후보' if mentions else '정보', 'confidence':'source-title/meta', 'url':r.get('url',''), 'published_at':r.get('published_at',''), 'collected_at':now})
