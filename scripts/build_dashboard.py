...
def ai_recommendation_section(region, recommendations):
    if isinstance(recommendations, list):
        recs = [r for r in recommendations if r.get('region') == region][:3]
    else:
        recs = (recommendations or {}).get(region, [])[:3]
    title = '국내주식 AI 추천 Top3' if region == 'domestic' else '미국주식 AI 추천 Top3'
