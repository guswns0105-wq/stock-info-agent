...
def ai_recommendation_section(region, recommendations):
    ...
    title = '국내주식 AI 관심종목 Top5' if region == 'domestic' else '미국주식 AI 관심종목 Top5'
    empty = '국내 AI 관심종목을 고를 데이터가 아직 없습니다.' if region == 'domestic' else '미국/해외 AI 관심종목을 고를 데이터가 아직 없습니다.'
    html = ''.join(ai_recommendation_card(rec) for rec in recs) or f'<p class="empty">{empty}</p>'
    return f'''<section class="ai-top-section"><h2>{title}</h2><p class="section-note">현재가가 계산상 적정가격 이하인 종목만 관심 후보로 보여줍니다. 조건을 통과한 종목이 부족하면 5개보다 적게 표시됩니다. “알림 걸어둘 가격”은 다시 볼 만한 가격, “비싸 보이기 시작하는 가격”은 과열을 의심할 가격입니다. 매수·매도 지시는 아닙니다.</p><div class="ai-rec-grid">{html}</div></section>'''
...
def methodology_section(methodology, yt_items, transcript_items):
    ...
    local_pack_html = ''.join(f'<article class="analysis-evidence-card"><b>{esc(public_text(e.get("name")))}</b><span>{esc(public_text(e.get("evidence")))}</span><small>{esc(public_text(e.get("dashboard_use")))}</small></article>' for e in evidence[:8])
...
def youtuber_card(stat, region=None):
    ...
    video_rows = []
    for video in stat.get('videos', [])[:20]:
        ...
        ocr_excerpt = esc(public_text(video.get('ocr_excerpt') or ''))
        ocr_line = f'<em class="ocr-snippet">화면근거: {ocr_excerpt}</em>' if ocr_excerpt else ''
        url = esc(video.get('url') or '')
        link = f'<a href="{url}" target="_blank" rel="noreferrer">보기</a>' if url else ''
        video_rows.append(f'<li><span class="yt-video-kind">{kind}</span><b>{region_text(video.get("title"), region)}</b><div>{tags}</div><small>{conf} 기반{ocr_badge} · {short_date(video.get("published_at"))} {link}</small>{ocr_line}</li>')
