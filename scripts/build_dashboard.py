...
def ai_recommendation_section(recommendations):
    if isinstance(recommendations, list):
        domestic = [r for r in recommendations if r.get('region') == 'domestic'][:3]
        global_recs = [r for r in recommendations if r.get('region') == 'global'][:3]
    else:
        domestic = (recommendations or {}).get('domestic', [])[:3]
        global_recs = (recommendations or {}).get('global', [])[:3]
    domestic_html = ''.join(ai_recommendation_card(r) for r in domestic) or '<p class="empty">국내 AI 추천 후보를 산정할 데이터가 아직 없습니다.</p>'
    global_html = ''.join(ai_recommendation_card(r) for r in global_recs) or '<p class="empty">미국/해외 AI 추천 후보를 산정할 데이터가 아직 없습니다.</p>'
    return f'''<section class="ai-top-section"><h2>AI 추천종목 Top3</h2><p class="section-note">출처 언급량, PER/PBR/ROE, 적정가 대비 여력, 1년 일봉 차트(SMA·52주 범위·20/60일 모멘텀)를 합산한 검토용 순위입니다. 매수·매도 지시가 아니라 가격 알림/분할 검토 기준입니다.</p>
    <h3 class="ai-market-title">국내주식 AI 추천 Top3</h3><div class="ai-rec-grid">{domestic_html}</div>
    <h3 class="ai-market-title">미국주식 AI 추천 Top3</h3><div class="ai-rec-grid">{global_html}</div></section>'''
...
<main><div class="notice">이 페이지는 출처 발언과 공개 재무지표를 구조화한 정보 대시보드입니다. Hermes 판단은 PER/PBR/ROE/EPS/마진과 차트 기반의 기계적 스크리닝이며 투자 조언이나 수익 보장이 아닙니다. 국내 Top3와 미국주식 Top3의 적정매수가·적정매도가는 가격 알림/검토 기준입니다. 실제 매매 전에는 현재가·공시·실적·수급을 별도로 확인해 주세요.</div>{ai_recommendation_section(ai_recommendations)}{section('domestic', common, items, ystats, valuations)}{section('global', common, items, ystats, valuations)}</main>
