...
def news_section(items):
    ...
    return f'''<section id="news" class="news-section"><h2>시간별 뉴스 브리핑</h2><p class="section-note">RSS/Google News/금융 뉴스에서 가져온 최신 헤드라인입니다. 종목 추천 점수에는 출처 언급·재무·차트가 우선 반영되고, 뉴스는 리스크/촉매 확인용으로 봅니다.</p><div class="news-grid">{''.join(groups)}</div></section>'''
...
<nav class="tabs"><a href="#news">뉴스</a><a href="#method">방법론</a><a href="#domestic">국내 AI추천</a><a href="#global">미국 AI추천</a></nav>
<main><div class="notice">이 페이지는 출처 발언, 공개 재무지표, 추가 분석 근거, 최신 뉴스를 구조화한 정보 대시보드입니다. Hermes 판단은 PER/PBR/ROE/EPS/마진과 차트 기반의 기계적 스크리닝이며 투자 조언이나 수익 보장이 아닙니다. 각 탭의 AI Top5는 해당 시장 종목만 보여주며 적정매수가·적정매도가는 가격 알림/검토 기준입니다. 실제 매매 전에는 현재가·공시·실적·수급을 별도로 확인해 주세요.</div>{news_section(items)}<div id="method">{methodology_section(methodology, yt_items, transcript_items)}</div>{section('domestic', common, items, ystats, valuations, ai_recommendations)}{section('global', common, items, ystats, valuations, ai_recommendations)}</main>
