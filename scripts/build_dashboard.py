...
def ai_recommendation_section(recommendations):
    html = ''.join(ai_recommendation_card(r) for r in recommendations[:3]) or '<p class="empty">AI 추천 후보를 산정할 데이터가 아직 없습니다.</p>'
    return f'<section class="ai-top-section"><h2>AI 추천종목 Top3</h2><p class="section-note">출처 언급량, PER/PBR/ROE, 적정가 대비 여력, 1년 일봉 차트(SMA·52주 범위·20/60일 모멘텀)를 합산한 검토용 순위입니다. 매수·매도 지시가 아니라 가격 알림/분할 검토 기준입니다.</p><div class="ai-rec-grid">{html}</div></section>'
